...
async function fetchData() {
  jwtLogin().then((res) => (res && hasRights(Role.User) ? sendRequest() : router.push({ name: routes.login.name })));
}
...
const actions = [
  {
    ...
    handler: (row: Record<string, any>) =>
      ...
      router.push(router.resolve({ name: 'user-orders-view', query: { id: row._id } })),
  },
];

function colFilter(col: TableColumn) {
  return ['_id', 'createdAt', 'status'].includes(col.key) && col.key.indexOf('_') === -1;
}
